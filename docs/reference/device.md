...
键值：key使用8个bit的数字表示，范围从0～255，可以表示256个不同的key；
...
SDK提供一套独立的编码和解码接口，当开发者认为需要编码为KLV时，调用我们的接口编码为二进制，当开发者认为需要解码时，调用SDK中的解码接口，解码为json格式的kv。
...
厂商管理后台在创建产品时，需要添加数据点，添加数据点时需要指明选择的数据点类型【二进制/json/KLV】。一个产品只能选择一种数据类型。

添加数据点完成后，云端自动生成KLV的数据点格式，格式如下：

```
{
    "name": "foosball",             //产品名称
"protocolType": "KLV",         //消息协议json/KLV/binary
“communication”:”wifi”         //通信协议 wifi/bluetooth
    "entities": [
        {
            "attrs": [
                {
                    "display_name": "空调温度", //显示名称
                    "name": "game_control",    //数据点名称，作为json格式的key
                    "data_type": "bool",    //数据类型bool/int32/int64/float/double/string/binary/klv
                    “data_len”: 1          //数据长度，单位字节，当数据类型为klv时，此值为0xffff。
                    "type": "status_writable",   //数据属性 可写/只读/故障/告警
                    "id": 0,                   //json格式的key映射到klv时的key，范围0～255
                    "desc": "....."              //描述
                }, 
	]
	}
