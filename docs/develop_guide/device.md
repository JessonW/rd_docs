...
对于市面上常见的WiFi模块，AbleCloud都进行了适配。WiFi模块上电后可以自动连接到AbleCloud云端。
开发者只需要按照AbleCloud的协议通过串号和WiFi模块进行交互即可。

对于以下模组长的WiFi模块已经完成了适配：
卓立合，江波龙，思存，航锐，汉枫，庆科，RAK，联盛德

对于一下WiFi芯片已经完成了适配：
MTK7681，QCA4004，Marvel-88MC500
